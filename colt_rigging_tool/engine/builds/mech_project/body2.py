...
def initChar(asset_name=None, debug = 1):
    ...
    if debug:
        ...
        mc.setAttr("{}.modelDisplay".format("mech_global_ctrl"), 0)
        [mc.setAttr("{}.drawStyle".format(itm), 0) for itm in mc.ls(type="joint")]
    else:
        mc.setAttr("{}.jointsDisplay".format("mech_global_ctrl"), 2)
        mc.setAttr("{}.modelDisplay".format("mech_global_ctrl"), 2)
        [mc.setAttr("{}.drawStyle".format(itm), 2) for itm in mc.ls(type="joint")]
...
######################################################################################################
# INIT
ASSET = initChar(asset_name="mech", debug = 1)
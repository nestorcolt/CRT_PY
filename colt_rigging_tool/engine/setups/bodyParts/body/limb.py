...
class Limb(object):
    ...
    def makeFK(self, simple_fk=False, world_orient=False):
        ...
        #
        if world_orient:
            cons = cmds.parentConstraint(fk_hier[-1], q=True)
            cmds.delete(cons)
            cmds.rotate(0,fk_controls[-1].root, rotateY=True, absolute=True, worldSpace=True)
            cmds.parentConstraint(fk_controls[-1].control, fk_hier[-1], mo=True)
    ...
    def makeIK(self, world_orient=False):
        ...
        cmds.orientConstraint(ik_control.control, ik_hier[-1], mo=True)
